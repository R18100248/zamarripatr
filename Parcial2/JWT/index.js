...
app.get('/sistema1',verificarToken,function(req,res,next) //Verificar si ese token es valido
{
    res.json({mensaje:"Acceso concedido a ruta sistema1"});
})

app.listen(8081,function()
{
    console.log("Servidor Express escuchando en el puerto 8081 - Rogelio Zamarripa Treviño, 18100248")
});
...
function verificarToken(req,res,next)
{
    ...
    jsonwebtoken.verify(token,'claveSecreta',function(err, decoded)
    {
        ...
        if(err)
        {
            res.json({Error:"Acceso NO concedido a ruta sistema1"});
        }
        else
        {
            next();
